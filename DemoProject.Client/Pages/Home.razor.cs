...
namespace DemoProject.Client.Pages
{
    public partial class Home
    {
        ...
        private async Task OnExecute(HtmlEditorExecuteEventArgs args)
        {
            ...
            else if(args.CommandName == "DeleteNote")
            {
        ...
        private async Task ConfirmDelete()
        {
            ...
            if (string.IsNullOrEmpty(url))
            {
                NotificationServiceWrapper.ShowError("Invalid note URL.");
                return;
            }

            var success = await NoteService.DeleteNoteAsync(url);
            if (success)
            {
                NotificationServiceWrapper.ShowSuccess("Note deleted.");
                // navigate to home/new note
                Nav.NavigateTo("/", true);
            }
            else
            {
                NotificationServiceWrapper.ShowError("Error deleting note.");
            }
        }
    }
}
